...
def fetch_movie_details(movie_id):
    try:
        url = f"https://api.themoviedb.org/3/movie/{movie_id}?api_key=2f27df673abb898064408efdfd66c03b&language=en-US&append_to_response=credits"
...
def fetch_trailer(movie_id):
    try:
        url = f"https://api.themoviedb.org/3/movie/{movie_id}/videos?api_key=2f27df673abb898064408efdfd66c03b&language=en-US"
        response = requests.get(url, timeout=10)
        response.raise_for_status()
        data = response.json()
